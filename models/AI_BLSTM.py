...
class Model(nn.Module):
    ...
    def forward(self, q1, q2, q1_len, q2_len):
        ...
        for i in range(len(q1_len)):
            ...
            joint = torch.tanh(self.fc0(joint.view(-1, joint.size()[-1])))
            #  func.tanh()
            ...
            if (len(s2.shape) == 1):
                s2 = torch.reshape(s2,(1,300))

            a1 = self.att(torch.cat([r1i, s1], 1))
            a2 = self.att(torch.cat([r2i, s2], 1))
        
            a1 = func.softmax(a1.t(),dim=1).t().expand_as(s1)
            a2 = func.softmax(a2.t(),dim=1).t().expand_as(s2)
...
def run_batch(b_data, b_lengths, model, optimizer=None):
    ...
    # .cuda()    
    q1 = Variable(torch.from_numpy(b_data['q1']))
    q2 = Variable(torch.from_numpy(b_data['q2']))
    ...
    if optimizer:
        ...
        # .cuda()
        target = Variable(torch.from_numpy(b_data['y']).type(torch.long))
        optimizer.zero_grad()
...
def run():
    print('Loading data...')
    # with open('eng_data_emb', 'rb') as f:
    with open('../data/paraphrases_emb', 'rb') as f:
